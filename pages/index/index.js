var cartListJson = require("../../data/cartLists.js");
...
Page({
  ...
  onShow() {
		this.setData({
			cartLists:cartListJson.cartLists
		});
    wx.showToast({
      title: '加载中',
      icon: "loading",
...
// 循环数组数据，判断----选中/未选中[selected]
  ...
  btn_add(e) {
    ...
    let list = this.data.cartLists;
    // 获取商品数量
    let num = list[index].count.quantity;
		let max=list[index].count.max;
  ...
  btn_submit_order: function () {
    ...
    //   })
		var hasSlected;
		hasSlected=this.data.cartLists.find((el)=>{
  ...
  count_price() {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      if (list[i].selected) {
        // 所有价格加起来 count_money
        total += list[i].count.quantity * list[i].price;
      }
    }
    // 最后赋值到data中渲染到页面
    this.setData({
